...
function resolveDatabaseAdapter(adapter, resolver) {
...
async function getDatabase(database) {
  ...
  return (type === "bunsqlite" ? import('db0/connectors/bun-sqlite') : isSqlite3Available() ? import('db0/connectors/sqlite3') : import('db0/connectors/better-sqlite3')).then((m) => {
    const connector = m.default || m;
    return connector({ path: database.filename });
  });
}
const _localDatabase = {};
async function getLocalDatabase(database, connector) {
  ...
  const fetchDevelopmentCacheForKey = async (key) => {
    return await db.prepare("SELECT * FROM _development_cache WHERE id = ?").get(key);
  };
  const insertDevelopmentCache = async (id, checksum, parsedContent) => {
    deleteDevelopmentCache(id);
    db.prepare(`INSERT INTO _development_cache (id, checksum, parsedContent) VALUES (?, ?, ?)`).run(id, checksum, parsedContent);
  };
