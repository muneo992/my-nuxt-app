...
function resolveDatabaseAdapter(adapter, opts) {
...
async function getDatabase(database, opts) {
  ...
  return import(findBestSqliteAdapter(opts)).then((m) => {
    const connector = m.default || m;
    return connector({ path: database.filename });
  });
}
const _localDatabase = {};
async function getLocalDatabase(database, { connector, nativeSqlite } = {}) {
...
function findBestSqliteAdapter(opts) {
